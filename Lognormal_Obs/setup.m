...
Y = exp(0.5*abs(XT-2.5) + obsErr*randn(size(XT))); % Add obs error
Y(Y <= 0) = 1E-12; % ksdensity can't handle values on the boundary of the support
                   % But these values only exist due to roundoff anyways
